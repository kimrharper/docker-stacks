...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the krh username if it exists
    if id krh &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER krh
    fi
    ...
    if [[ "$NB_USER" != "krh" ]]; then
        ...
        if [[ ! -e "/home/$NB_USER" ]]; then
            ...
            mv /home/krh "/home/$NB_USER"
        fi
        # if workdir is in /home/krh, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/krh/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u krh)" && "$NB_GID" == "$(id -g krh)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^krh:/nayvoj:/" > /tmp/passwd
                echo "krh:x:$(id -u):$(id -g):,,,:/home/krh:/bin/bash" >> /tmp/passwd
